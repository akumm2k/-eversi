...
public class ReversiAgent {
     ...
     */
    private final ReversiGame model;

    /**
     * the player ID the agent takes
     */
    private final Disk agentDisk;


    /**
     * Constructor for the agent
     * @see ReversiAgent#agentDisk
     * @param model the game model the agent will play on
     * @param depth the max depth for minimax search
     * @param agentDisk the player ID the agent takes
     */
    public ReversiAgent(ReversiGame model, int depth, Disk agentDisk) {
        this.agentDisk = agentDisk;
        this.model = model;
        this.depth = depth;
    }
    ...
    public Coordinate findBestMove() {
        ...
        for (Coordinate move : moves) {
            ReversiGame clonedModel = ReversiGame.getClone(this.model);
     ...
     */
    private int miniMax(ReversiGame gameState, int currDepth, boolean maximizingPlayer, int alpha, int beta) {
     ...
     */
    private int evaluate(final ReversiGame gameState) {
     ...
     * @return the heuristic evaluation
     */
    private int endToEndStableOccupation(final ReversiGame gameState) {
        ...
        final CompletableFuture<Integer> numFullRowsFuture = CompletableFuture.supplyAsync(() ->
                ...
                .filter(row ->
                        IntStream.of(row).allMatch(playa -> playa == this.agentDisk.getValue())
                ).count());
        ...
        final CompletableFuture<Integer> numFullColsFuture = CompletableFuture.supplyAsync(() ->
                ...
                .filter(col ->
                        col.allMatch(playa -> playa == this.agentDisk.getValue())
                ).count());
     ...
     * @return the heuristic evaluation
     */
    private int countMyPieces(final ReversiGame gameState) {
        final int stratVal = 5;
        int cnt = 0;

        for (int[] row: gameState.getBoard()) {
            for (int playa: row) {
                if (playa == this.agentDisk.getValue()) {
                    cnt++;
                }
            }
        }
     ...
     * @return agentID
     */
    public Disk getAgentDisk() {
        return this.agentDisk;
    }
}
